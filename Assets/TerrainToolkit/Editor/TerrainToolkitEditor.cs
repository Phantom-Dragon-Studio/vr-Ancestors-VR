		...
		switch (terrain.toolModeInt) {
			...
			switch (terrain.generatorTypeInt) {
				...
				if (GUI.Button(buttonRect, "Generate Voronoi Features")) {
					...
					Undo.RegisterCompleteObjectUndo(terrData, "Terrain Generator");
			...
			// -------------------------------------------------------------------------------------------------------- TEXTURING TOOLS
			case 2:
			Terrain terrr = (Terrain) terrain.GetComponent(typeof(Terrain));
			if (terrr == null) {
				return;
			}
			TerrainData terrrData = terrr.terrainData;
			terrain.splatPrototypes = terrrData.splatPrototypes;
			EditorGUILayout.Separator();
			float mouseX;
			EditorGUILayout.BeginHorizontal();
			GUI.skin = terrain.guiSkin;
			...
			EditorGUILayout.EndHorizontal();
			if (GUI.changed) {
				terrrData.splatPrototypes = terrain.splatPrototypes;
			}
